...
EVT 给出的 `event_strength` 通过 `phi` 映射为 `S_k` 累积权重：
...
| `max_S`, `mean_S`, `integral_S` | FLOAT | S_k 衰减强度统计量 |
...
```python
f_i = ln(|z_i - 0.5| + 1)          # 旧方案，已废弃
C_k = Σ f_i · exp(-λ · gap)        # 旧方案，已废弃
```
...
| `pwm_extreme/events.py:extract_segments()` | 改为 S_k > 1 + 单个 normal bridge 规则 |
| `pwm_extreme/events.py:z_i 计算` | 替换为 EVT-based phi_i |
>>>>>>> feature/pwm-sk-bridge
