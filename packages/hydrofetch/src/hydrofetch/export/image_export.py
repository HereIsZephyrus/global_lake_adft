...
def build_era5_daily_image(
    ...
    Filters the collection to a one-day window, takes the first image, selects
    the requested bands, and clips to *region*.

    Args:
        spec: Image export spec (supplies ``asset_id`` and ``band_names``).
        day: Calendar date.
        region: GeoJSON-like dict (Feature or Geometry) defining the AOI.
...
def submit_image_export(
    ...
    *,
    ...
    Args:
        ...
        region: GeoJSON-like dict (Feature or Geometry) for the AOI.
    ...
    log.info(
        "GEE export submitted: task_id=%s export_name=%s day=%s",
        task.id,
        export_name,
        day,
    )
