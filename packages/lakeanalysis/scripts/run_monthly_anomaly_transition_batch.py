"""Run chunked DB batch execution for monthly anomaly transition workflow."""
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "monthly_anomaly_transition"


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for batch execution."""
    parser = argparse.ArgumentParser(
        description="Run monthly anomaly transition workflow on DB data in chunks.",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
...
def run(args: argparse.Namespace):
    ...
    return run_monthly_transition_batch(config)


def main() -> None:
    """CLI entry point."""
    args = parse_args()
    Logger("run_monthly_anomaly_transition_batch")
