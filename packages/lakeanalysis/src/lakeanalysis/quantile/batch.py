...
def process_chunk_lakes(
    ...
    *,
    ...
    run_single_fn: Callable[..., QuantileResult] | None = None,
) -> ChunkProcessPayload:
...
def _persist_chunk_payload(payload: ChunkProcessPayload) -> None:
    with series_db.connection_context() as conn:
        try:
            upsert_quantile_labels(conn, payload.label_rows, commit=False)
            upsert_quantile_extremes(conn, payload.extreme_rows, commit=False)
            upsert_quantile_abrupt_transitions(
