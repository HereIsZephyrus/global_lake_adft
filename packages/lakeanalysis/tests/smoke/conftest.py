...
def parquet_id_range(parquet_data_dir):
    """Discover id range from parquet area_quality data."""
    from lakesource.parquet.client import DuckDBClient
    client = DuckDBClient(data_dir=parquet_data_dir)
    try:
        df = client.query_df("SELECT MIN(hylak_id) AS mn, MAX(hylak_id) AS mx FROM area_quality")
        if df.empty or df["mn"].isna().all():
            pytest.skip("No data in area_quality parquet")
        return int(df["mn"].iloc[0]), int(df["mx"].iloc[0]) + 1
    except Exception:
