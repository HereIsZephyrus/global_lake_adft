...
def test_eot_calculator_runs_lake_dataset_batch() -> None:
    dataset = LakeDataset(
    ...
    assert "eot_run_status" in rows_by_table
    assert len(rows_by_table["eot_run_status"]) == 2
    assert {row["hylak_id"] for row in rows_by_table["eot_run_status"]} == {1, 2}
